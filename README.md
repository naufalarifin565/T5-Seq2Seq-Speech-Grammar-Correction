...
```bash
git clone [https://github.com/naufalarifin565/T5-Seq2Seq-Speech-Grammar-Correction.git](https://github.com/naufalarifin565/T5-Seq2Seq-Speech-Grammar-Correction.git)
cd T5-Seq2Seq-Speech-Grammar-Correction
...
pip install transformers openai-whisper pandas numpy scikit-learn jupyter ipykernel
pip install google-generativeai  # Required if running GEMINI2.0Flash.py
...
Since this project primarily uses Jupyter Notebooks (`.ipynb`), you need to launch the Jupyter environment.
...
This will open a dashboard in your web browser.
2. **Usage Guide:**
* **Step 1 (ASR):** Open `WHISPER.ipynb` to process audio files into text.
* **Step 2 (Training):** Open `Training_NLP_T5.ipynb` to train the T5 model on the provided CSV datasets.
* **Step 3 (Scenarios):** Explore the `NLP_Dataset_Skenario_X.ipynb` notebooks to see how the model handles different data distributions.
* **Step 4 (Demo):** Open `Interface_FP_NLP (1).ipynb` to test the full pipeline interactively (Speech ➔ Text ➔ Corrected Text).



### ⚠️ Hardware Requirement Note

* **GPU:** Highly recommended for training T5 and running Whisper (Medium/Large models).
* **CPU:** Sufficient for inference with smaller Whisper models (Base/Tiny) and T5-Small.
